...
def sharedprepare(self):
    ...
    neo4j_accoount['conf_account_stanza_name'] = get_splunklib_config( self, conf_accounts_page, desired_account_stanza, 'name',             str, "Could not find the specified account id \"%s\"" % ( desired_account_stanza ) )
    neo4j_accoount['username']                 = get_splunklib_config( self, conf_accounts_page, desired_account_stanza, 'account_username', str, "Could not find the specified account id \"%s\"" % ( desired_account_stanza ) )
    desired_neo4j_account_id = neo4j_accoount['conf_account_stanza_name']
    ...
    try:
        ...
        neo4j_accoount['storage_stanza_name'] = stanza_name
        neo4j_accoount['path'] = storage_password['path']
        neo4j_accoount['realm'] = storage_password['realm']
        neo4j_accoount['password'] = json.loads(storage_password['clear_password'])['account_password']
    ...
    return app_id, conf_uris_page, conf_accounts_page, neo4j_uri, neo4j_accoount, noact
#------------------   def prepare(): end               -----------------#